...
namespace net.raitichan.int_parameter_compressor.Pass {
    internal class IntParameterCompressingPass : Pass<IntParameterCompressingPass> {
        ...
        protected override void Execute(BuildContext context) {
            ...
            VRCAvatarDescriptor descriptor = context.VRChatAvatarDescriptor();

            if (descriptor.expressionParameters == null) return;
            VRCExpressionParameters parameters = descriptor.expressionParameters;

            this.FindTargetParameter(descriptor);

            if (this._maxParameterWarning != null) {
                ErrorReport.ReportError(this._maxParameterWarning);
            }

            if (this._maxValueDict.Count <= 0) return;

            AnimatorController? controller = GetFxLayer(descriptor);
            ...
            foreach ((string targetParameter, int value) in this._maxValueDict) {
                int useBitSize = GetRequiredBitSize(value);
                ...
                // Add Parameter
                // TODO: Detect generated parameter name conflicts before adding expression or animator parameters.
            ...
            // Add parameter
            parameters.parameters = parameters.parameters.Concat(appendVrcParameters).ToArray();

            {
                // Process FX Layer
                // IsLocal Parameter Check
                bool isLocalParamIsFloat = false;
                AnimatorControllerParameter? isLocalParameter = controller.parameters.FirstOrDefault(parameter => parameter.name == "IsLocal");
...

                {
                    ...
                    foreach ((string parameterName, int maxValue) in this._maxValueDict) {
                        ...
                        int useBitSize = GetRequiredBitSize(maxValue);
                        ...
                        while (stateCunt > 4) {
                            ...
                            for (int i = 0; i < stateCunt; i++) {
                                AnimatorState encodeState = new()
                                    { name = $"encode-node-{i * layerMul}-{(i + 1) * layerMul - 1}", writeDefaultValues = this._writeDefault };
                                encodeStates.Add(encodeState);
                                AssetDatabase.AddObjectToAsset(encodeState, context.AssetContainer);

                                AnimatorState decodeState = new()
                                    { name = $"decode-node-{i * layerMul}-{(i + 1) * layerMul - 1}", writeDefaultValues = this._writeDefault };
...

                        {
                            ...
                            for (int stateIndex = 0; stateIndex < topEncodeStates.Count; stateIndex++) {
                                ...
                                }
                                {
                                    ...
                                    transition.AddCondition((stateIndex & 1) == 1 ? AnimatorConditionMode.If : AnimatorConditionMode.IfNot, 1,
                                        $"{parameterName}.bit_{currentBit}");
                                    if ((useBitSize & 1) == 0) {
                                        transition.AddCondition((stateIndex & 2) == 2 ? AnimatorConditionMode.If : AnimatorConditionMode.IfNot, 1,
                                            $"{parameterName}.bit_{currentBit + 1}");
                                    }
                                }
                            }
                        }
                        ...
                        while (encodeStatesStack.Count > 0) {
                            ...
                            if (encodeStatesStack.TryPeek(out List<AnimatorState> childEncodeStates) &&
                                decodeStatesStack.TryPeek(out List<AnimatorState> childDecodeStates)) {
                                ...
                                for (int stateIndex = 0; stateIndex < childEncodeStates.Count; stateIndex++) {
                                    int parentStateIndex = stateIndex / 4;
                                    {
                                        AnimatorStateTransition transition =
                                            currentEncodeStates[parentStateIndex].AddTransition(childEncodeStates[stateIndex]);
                                        transition.duration = 0;
                                    ...
                                    {
                                        AnimatorStateTransition transition =
                                            currentDecodeStates[parentStateIndex].AddTransition(childDecodeStates[stateIndex]);
                                        transition.duration = 0;
                                ...
                                for (int stateIndex = 0; stateIndex < currentEncodeStates.Count; stateIndex++) {
                                    ...
                                    for (int bitCount = 0; bitCount < useBitSize; bitCount++) {
                                        ...
                                        transition.AddCondition(flag ? AnimatorConditionMode.IfNot : AnimatorConditionMode.If, 1,
                                            $"{parameterName}.bit_{bitCount}");
                                    }
                                }
                            }
                        }
        ...
        private static int GetRequiredBitSize(int maxValue) {
        ...
        private static AnimatorController? GetFxLayer(VRCAvatarDescriptor descriptor) {
            ...
            foreach (VRC_AvatarParameterDriver.Parameter parameter in drivers.SelectMany(driver => driver.parameters)
                ...
                switch (parameter.type) {
                    ...
                    case VRC_AvatarParameterDriver.ChangeType.Add:
                        ...
                        this.UnknownMaxParameterWarning
                            .AddUnknownMaxParameter(parameter.name, UnknownMaxParameterWarning.ReasonType.ParameterDriverAdd);
                        break;
                    ...
                    case VRC_AvatarParameterDriver.ChangeType.Copy:
                        if (parameter.convertRange) {
                            ...
                            this.UnknownMaxParameterWarning
                                .AddUnknownMaxParameter(parameter.name, UnknownMaxParameterWarning.ReasonType.ParameterDriverCopy);
                        }
                ...
                foreach (AnimatorCondition condition in transition.conditions
                    ...
                    switch (condition.mode) {
                        ...
                        case AnimatorConditionMode.Greater:
                            ...
                            val = (int)Mathf.Max(val, condition.threshold + 1);
                            break;
